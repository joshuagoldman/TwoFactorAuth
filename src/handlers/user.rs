...
use crate::{database, auth::{self, cache::Cache, models::SessionInfo}};
...
async fn verify_otp(otp: Path<String>,
                    ...
                    login_cache_data: Data<Arc<Mutex<Cache<Uuid>>>>) -> impl Responder {
    let addr = state.as_ref().addr.clone();
...
#[get("/user")]
async fn get_user(req_user: Option<ReqData<crate::auth::models::TokenClaims>> ,
                     state: Data<database::models::AppState>) -> impl Responder {
    let addr = state.as_ref().addr.clone();

    match addr.send(
        crate::actors::user::GetUser { 
            id: req_user.unwrap().into_inner().id
        }).await {
        Ok(Ok(user)) => HttpResponse::Ok().json(user),
        _ => HttpResponse::InternalServerError().json("Something went wrong")
...
#[get("/expired")]
async fn has_expired(credentials: BearerAuth,
                     login_cache_data: Data<Arc<Mutex<Cache<Uuid>>>>) -> impl Responder {
...
#[get("/logout")]
async fn logout(credentials: BearerAuth,
                     login_cache_data: Data<Arc<Mutex<Cache<Uuid>>>>) -> impl Responder {
    let jwt_secret: String = std::env::var("JWT_SECRET_OTP").expect("JWT_SECRET_OTP must be set!");
