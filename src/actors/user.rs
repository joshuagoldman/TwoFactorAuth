...
use crate::{
    ...
    database::models::{LoginResponse, NewUser, OtpResponse, User, UserResponse},
    schema,
};
...
impl Handler<Create> for DbActor {
    ...
    fn handle(&mut self, msg: Create, _: &mut Self::Context) -> Self::Result {
        ...
        let resp = diesel::insert_into(schema::users::dsl::users)
            .values(new_article)
            ...
            Ok(found_user) => {
                ...
                if is_valid {
                    let jwt_secret: Hmac<Sha256> =
                        Hmac::new_from_slice(self.config.jwt_secret_basic.as_bytes()).unwrap();
                    let claims = auth::models::TokenClaimsWithTime {
                        id: found_user.id,
...
impl Handler<VerifyOtp> for DbActor {
    ...
    fn handle(&mut self, msg: VerifyOtp, _: &mut Self::Context) -> Self::Result {
        ...
        let claims =
            auth::middle_ware::get_jwt_claims_with_time(&msg.token, &self.config.jwt_secret_basic)
                .unwrap();
        ...
        let found_user_res = users
            .filter(crate::schema::users::dsl::id.eq(claims.id))
            .get_result::<User>(&mut conn);

        match found_user_res {
            Ok(found_user) => {
                let mc = new_magic_crypt!(&self.config.secret_otp_encrypted, 256);
                let decrypt_secret = mc
                    .decrypt_base64_to_string(found_user.otp_secret_encrypted.unwrap())
                    .unwrap();
                ...
                if code == msg.otp {
                    let jwt_secret: Hmac<Sha256> =
                        Hmac::new_from_slice(self.config.jwt_secret_otp.as_bytes()).unwrap();
...
impl Handler<ChangePassword> for DbActor {
    ...
    fn handle(&mut self, msg: ChangePassword, _: &mut Self::Context) -> Self::Result {
        ...
        let res = diesel::update(schema::users::dsl::users)
...
impl Handler<RemoveUser> for DbActor {
    ...
    fn handle(&mut self, msg: RemoveUser, _: &mut Self::Context) -> Self::Result {
        ...
        let res = diesel::delete(schema::users::dsl::users)
            .filter(schema::users::dsl::id.eq(msg.id))
            .get_result::<User>(&mut conn);
...
impl Handler<GetUser> for DbActor {
    ...
    fn handle(&mut self, msg: GetUser, _: &mut Self::Context) -> Self::Result {
        ...
        let res = schema::users::dsl::users
            .filter(schema::users::dsl::id.eq(msg.id))
            .get_result::<User>(&mut conn);
