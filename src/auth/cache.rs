...
use crate::auth::models::SessionInfo;
...
#[cfg(test)]
mod tests {
    use crate::auth::{cache, models::SessionInfo};

    #[tokio::test]
    async fn test_cache_logged_in_within_time() {
        let app_cache: std::sync::Arc<tokio::sync::Mutex<cache::Cache<uuid::Uuid>>> = cache::Cache::new_app_cache("3 seconds".to_string());
    ...
    async fn test_cache_logged_in_not_within_time() {
        let app_cache: std::sync::Arc<tokio::sync::Mutex<cache::Cache<uuid::Uuid>>> = cache::Cache::new_app_cache("2 seconds".to_string());
